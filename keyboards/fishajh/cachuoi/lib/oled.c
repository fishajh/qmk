...
#define LOCK_DISPLAY_X 98
#define LOCK_DISPLAY_Y 0
led_t led_state = host_keyboard_led_state();
  if (led_state.caps_lock) {
    draw_rect_filled_soft(LOCK_DISPLAY_X + 0, LOCK_DISPLAY_Y + 11, 5 + (3 * 6), 9, PIXEL_ON, NORM);
    draw_string(LOCK_DISPLAY_X + 3, LOCK_DISPLAY_Y +1 +11, "CAP", PIXEL_OFF, NORM, 0);
  } else {
    draw_string(LOCK_DISPLAY_X + 3, LOCK_DISPLAY_Y +1 +11, "CAP", PIXEL_ON, NORM, 0);
  }
...
#define CLOCK_DISPLAY_Y 14
  ...
  if(is_pm){
    draw_string(CLOCK_DISPLAY_X + 42, CLOCK_DISPLAY_Y, "pm", PIXEL_ON, NORM, 1);
  } else{
    draw_string(CLOCK_DISPLAY_X + 42, CLOCK_DISPLAY_Y, "am", PIXEL_ON, NORM, 1);
  }
